...
static xf_task_err_t xf_task_mbus_find(uint32_t topic_id, xf_task_mtopic_t **topic);
static void mbus_task(xf_task_t task);

/* ==================== [Static Variables] ================================== */

static xf_task_list_t _topic_list = XF_TASK_LIST_HEAD_INIT(_topic_list);
...
xf_task_err_t xf_task_mbus_reg_topic_with_manager(xf_task_manager_t manager, uint32_t topic_id, uint32_t size)
{
    ...
    xf_task_mtopic_t *mtopic = (xf_task_mtopic_t *)xf_task_malloc(sizeof(xf_task_mtopic_t) + DEFAULT_QUEUE_COUNT * size);
    ...
    xf_task_list_init(&mtopic->node);
    xf_task_list_init(&mtopic->sub_list);
    xf_task_queue_init(&mtopic->pub_queue, buf, size, DEFAULT_QUEUE_COUNT);
    mtopic->id = topic_id;
    mtopic->size = size;
    xf_task_list_add_tail(&mtopic->node, &_topic_list);
...
xf_task_err_t xf_task_mbus_unreg_topic(uint32_t topic_id)
{
    ...
    if (xf_task_mbus_find(topic_id, &mtopic) == XF_TASK_ERR_NOT_FOUND) {
        XF_TASK_LOGE(TAG, "topic:%d not found", (int)topic_id);
        return XF_TASK_ERR_NOT_FOUND;
    }

    xf_task_list_for_each_entry_safe(msub, _msub, &mtopic->sub_list, xf_task_msub_t, node) {
        xf_task_list_del_init(&msub->node);
        xf_task_free(msub);
    }
...
{
    XF_TASK_ASSERT(data, XF_TASK_ERR_INVALID_ARG, TAG, "data must not be NULL");
    ...
    xf_task_err_t err = xf_task_queue_send(&mtopic->pub_queue, data, XF_TASK_QUEUE_SEND_TO_BACK);
...
{
    XF_TASK_ASSERT(mbus_cb, XF_TASK_ERR_INVALID_ARG, TAG, "mbus_cb must not be NULL");
    ...
    // 防止重复注册
    xf_task_list_for_each_entry(msub, &mtopic->sub_list, xf_task_msub_t, node) {
        if (msub->mbus_cb == mbus_cb) {
            XF_TASK_LOGD(TAG, "mbus_cb is exists!");
            return XF_TASK_ERR_INVALID_ARG;
        }
    }

    // 如果没有重复注册，则创建并加入链表
    msub = (xf_task_msub_t *)xf_task_malloc(sizeof(xf_task_msub_t));
    ...
    xf_task_list_init(&msub->node);

    msub->mbus_cb = mbus_cb;
    msub->user_data = user_data;

    xf_task_list_add_tail(&msub->node, &mtopic->sub_list);
...
xf_task_err_t xf_task_mbus_unsub(uint32_t topic_id, xf_task_mbus_func_t mbus_cb)
{
    ...
    XF_TASK_LOGE(TAG, "mbus_cb not found!");
...
static xf_task_err_t xf_task_mbus_find(uint32_t topic_id, xf_task_mtopic_t **topic)
{
    ...
    xf_task_list_for_each_entry(mtopic, &_topic_list, xf_task_mtopic_t, node) {
