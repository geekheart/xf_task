...
xf_task_err_t xf_task_queue_init(xf_task_queue_t *const queue, void *data, const size_t size, const size_t count)
{
    ...
    XF_TASK_ASSERT(size > 0, XF_TASK_ERR_INVALID_ARG, TAG, "size must more than 0");
    XF_TASK_ASSERT(count > 0, XF_TASK_ERR_INVALID_ARG, TAG, "count must more than 0");

    xf_task_bzero(queue, sizeof(xf_task_queue_t));

    queue->count = count;
    queue->size = size;
    queue->head = (uint8_t *)data;

    xf_task_err_t res = xf_task_queue_reset(queue);
    if (res != XF_TASK_OK) {
        XF_TASK_LOGE(TAG, "queue reset failed");
        return res;
    }
...
{
    XF_TASK_ASSERT(queue, false, TAG, "queue must not be NULL");
...
{
    XF_TASK_ASSERT(queue, 0, TAG, "queue must not be NULL");
...
{
    XF_TASK_ASSERT(queue, NULL, TAG, "queue must not be NULL");
...
{
    XF_TASK_ASSERT(queue, XF_TASK_ERR_INVALID_ARG, TAG, "queue must not be NULL");
    XF_TASK_ASSERT(pos == 1 || pos == 0, XF_TASK_ERR_INVALID_ARG,
...
{
    move_reader(queue);
    xf_task_memcpy(buffer, queue->reader, queue->size);
}

static void copy_data_to_queue(xf_task_queue_t *const queue, const void *item, const bool pos)
{
    if (0 == pos) {
        xf_task_memcpy((void *)queue->writer, item, queue->size);
        queue->writer += queue->size;
        if (queue->writer >= queue->tail) {
            queue->writer = queue->head;
        }
    } else {
        xf_task_memcpy((void *)queue->reader, item, queue->size);
