...
static xf_task_t xf_ntask_constructor(xf_task_manager_t manager, xf_task_func_t func, void *func_arg, uint16_t priority,
                                      void *config);
static xf_ntask_lc_t *xf_ntask_lc_find(xf_task_list_t *head, const char *name);
static xf_ntask_lc_t *xf_ntask_lc_create(xf_task_list_t *head, const char *name);
static void xf_ntask_args_remove_all(xf_task_list_t *head);
static void xf_ntask_lc_remove_all(xf_task_list_t *head);
...
{
    xf_ntask_handle_t *handle = (xf_ntask_handle_t *)task;
    xf_task_list_t *head = &handle->args_list;
    xf_ntask_args_t *args = (xf_ntask_args_t *)xf_task_malloc(sizeof(xf_ntask_args_t) + size);
    if (args == NULL) {
        return NULL;
    }
    ...
    args->name = name;
    args->size = size;
    xf_task_list_init(&args->node);

    xf_task_list_add_tail(&args->node, head);
...
void *xf_ntask_args_find(xf_task_t task, const char *name)
{
    ...
    xf_task_list_for_each_entry(item, head, xf_ntask_args_t, node) {
        if (item->name == name) {
            return item->args;
        }
    }
    xf_task_list_for_each_entry(item, head, xf_ntask_args_t, node) {
        if (xf_task_strcmp(item->name, name) == 0) {
            return item->args;
        }
    }
    return NULL;
...
xf_task_err_t xf_ntask_stack_load(xf_task_t task, const char *name, xf_ntask_stack_t *stack, uint32_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        xf_task_memcpy(stack[i].addr, (uint8_t *)item + addr_offset, stack[i].size);
...
xf_task_err_t xf_ntask_stack_save(xf_task_t task, const char *name, xf_ntask_stack_t *stack, uint32_t len)
{
    ...
    uint32_t addr_offset = 0;
    for (size_t i = 0; i < len; i++) {
        xf_task_memcpy((uint8_t *)item + addr_offset, stack[i].addr, stack[i].size);
        addr_offset += stack[i].size;
    }
...
static xf_task_t xf_ntask_constructor(xf_task_manager_t manager, xf_task_func_t func, void *func_arg, uint16_t priority,
                                      void *config)
{
    xf_ntask_handle_t *task = (xf_ntask_handle_t *)xf_task_malloc(sizeof(xf_ntask_handle_t));
    ...
    xf_task_list_init(&task->lc_list);
    xf_task_list_init(&task->args_list);
    task->compare = NULL;
    task->status = XF_NTASK_NONE;

    XF_TASK_BITS_SET1(task->base.signal, XF_TASK_SIGNAL_EVENT);
...
static void xf_ntask_args_remove_all(xf_task_list_t *head)
{
    ...
    xf_task_list_for_each_entry_safe(item, _item, head, xf_ntask_args_t, node) {
        xf_task_list_del_init(&item->node);
        xf_task_free(item);
    }
}
...
static xf_ntask_lc_t *xf_ntask_lc_create(xf_task_list_t *head, const char *name)
{
    xf_ntask_lc_t *lc = (xf_ntask_lc_t *)xf_task_malloc(sizeof(xf_ntask_lc_t));
    if (lc == NULL) {
        return NULL;
    }

    xf_task_list_init(&lc->node);
    lc->lc = 0;
    lc->name = name;

    xf_task_list_add_tail(&lc->node, head);
...
{
    xf_ntask_lc_t *item;
    xf_task_list_for_each_entry(item, head, xf_ntask_lc_t, node) {
        if (item->name == name) {
            return item;
        }
    }
...
static void xf_ntask_lc_remove_all(xf_task_list_t *head)
{
    ...
    xf_task_list_for_each_entry_safe(item, _item, head, xf_ntask_lc_t, node) {
        xf_task_list_del_init(&item->node);
        xf_task_free(item);
    }
}
