...
typedef struct _xf_ctask_queue_handle_t {
    ...
    xf_task_list_t  send_waiting;
    xf_task_list_t  receive_waiting;
...
void xf_ctask_delay_with_manager(xf_task_manager_t manager, uint32_t delay_ms)
{
    ...
    if (task->base.type != XF_TASK_TYPE_CTASK) {
        XF_TASK_LOGE(TAG, "only ctask can use this function:%d", task->base.type);
        return;
    }
...
xf_ctask_queue_t xf_ctask_queue_create_with_manager(xf_task_manager_t  manager, const size_t size, const size_t count)
{
    ...
    XF_TASK_ASSERT(size, NULL, TAG, "size must not be 0");
    XF_TASK_ASSERT(count, NULL, TAG, "count must not be 0");

    xf_ctask_queue_handle_t *ctask_queue = (xf_ctask_queue_handle_t *)xf_task_malloc(sizeof(xf_ctask_queue_handle_t) +
                                           size * count);
    ...
    xf_task_bzero(ctask_queue, sizeof(xf_ctask_queue_handle_t) + size * count);
    ...
    xf_task_list_init(&ctask_queue->receive_waiting);
    xf_task_list_init(&ctask_queue->send_waiting);
...
{
    XF_TASK_ASSERT(queue, XF_TASK_RETURN_VOID, TAG, "queue must not be NULL");
...
{
    XF_TASK_ASSERT(queue, XF_TASK_ERR_INVALID_ARG, TAG, "queue must not be NULL");
    XF_TASK_ASSERT(buffer, XF_TASK_ERR_INVALID_ARG, TAG, "buffer must not be NULL");

    xf_task_queue_t *_queue = &((xf_ctask_queue_handle_t *)queue)->queue;
    xf_task_list_t *send_waiting = &((xf_ctask_queue_handle_t *)queue)->send_waiting;
    xf_task_list_t *receive_waiting = &((xf_ctask_queue_handle_t *)queue)->receive_waiting;
    ...
    xf_task_list_t *queue_node = &((xf_ctask_handle_t *)task)->queue_node;
    xf_ctask_handle_t *receive_task, *_receive_task;

    // 只有ctask才能调用
    if (XF_TASK_STATE_RUNNING != task->state) {
        XF_TASK_LOGE(TAG, "task state must RUNNING");
    ...
    while (1) {
        if (xf_task_queue_is_full(_queue)) {
            xf_task_list_add_tail(queue_node, send_waiting);
            ...
            xf_task_list_for_each_entry_safe(receive_task, _receive_task, receive_waiting, xf_ctask_handle_t, queue_node) {
                xf_task_list_del_init(&receive_task->queue_node);
...
xf_task_err_t xf_ctask_queue_receive(xf_ctask_queue_t queue, void *buffer, uint32_t timeout)
{
    ...
    while (1) {
        if (xf_task_queue_is_empty(_queue)) {
            xf_task_list_add_tail(queue_node, receive_waiting);
            ...
            xf_task_list_for_each_entry_safe(send_task, _send_task, send_waiting, xf_ctask_handle_t, queue_node) {
                xf_task_list_del_init(&send_task->queue_node);
                                      ...
                                      void *config)
{
    ...
    XF_TASK_ASSERT(stack_size > 0, NULL, TAG, "args must more than 0");

    xf_ctask_handle_t *task = (xf_ctask_handle_t *)xf_task_malloc(sizeof(xf_ctask_handle_t) + stack_size);
    ...
    xf_task_list_init(&task->queue_node);
...
static void xf_ctask_reset(xf_task_t task)
{
    ...
    xf_task_list_del_init(&handle->queue_node);
...
static void xf_ctask_yield(xf_task_manager_t manager)
{
    ...
    if (XF_TASK_STATE_RUNNING != task->base.state) {
        XF_TASK_LOGD(TAG, "task must running");
        return ;
    }
...
static void xf_ctask_resume(xf_task_manager_t manager)
{
    ...
    if (XF_TASK_STATE_RUNNING == task->base.state || XF_TASK_STATE_DELETE == task->base.state) {
        XF_TASK_LOGD(TAG, "task must not running and delete");
        return;
    }
