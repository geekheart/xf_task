set_symbols("debug")

-- xf_task所有的内容
function add_xf_task() 
    add_files("src/kernel/*.c")
    add_files("src/port/*.c")
    add_files("src/utils/*.c")
    add_files("src/task/*.c")
    add_includedirs("src")
end

-- xf_task移植的内容
function add_port() 
    add_files("port/*.c")
    add_includedirs("port")
    add_files("port/asm/jump_gas.S")
    add_files("port/asm/make_gas.S")
end

-- 模板化添加示例工程
function add_target(name)
    target(name)
        set_kind("binary")
        add_cflags("-Wall")
        add_files(string.format("example/%s/*.c", name))
        add_includedirs(string.format("example/%s", name))
        if name == "lwip_concurrency" then
            add_includedirs("third_party/lwip/src/include")
            add_includedirs("third_party/lwip/contrib/ports/unix/port/include")
            add_files("third_party/lwip/src/core/*.c")
            add_files("third_party/lwip/src/core/ipv4/*.c")
            add_files("third_party/lwip/src/netif/ethernet.c")
            add_files("third_party/lwip/contrib/ports/unix/port/netif/tapif.c")
        end
        add_xf_task()
        add_cflags("-O0")
        add_port()
end

-- 扫描example目录下的文件夹名
for _, dir in ipairs(os.dirs("example/*")) do
    local name = path.basename(dir)
    add_target(name)
end

-- unity tests
target("xf_task_tests")
    set_kind("binary")
    add_cflags("-Wall", "-O0")
    add_files("tests/*.c")
    add_files("third_party/unity/src/unity.c")
    add_includedirs("tests", "third_party/unity/src")
    add_xf_task()
